...
const GVXe = () => {
  ...
  const [form, setForm]         = useState(emptyBaoLoi)
  ...
  const openBaoLoi = (lich) => {
    setForm({ ...emptyBaoLoi, xe_id: lich.xe?.id || '', lich_hoc_id: lich.id })
    setShowModal(true)
  }
  ...
  return (
    <div className="gv-xe">
      <div className="page-header">
        ...
        <button className="btn btn-warning" onClick={() => { setForm(emptyBaoLoi); setShowModal(true) }}>
          ⚠️ Báo lỗi xe
        </button>
      </div>
      ...
      <div className="xe-tabs">
        <button className={`xe-tab ${tab==='xe'?'active':''}`} onClick={() => setTab('xe')}>🚗 Xe Được Cấp</button>
        <button className={`xe-tab ${tab==='lich_su'?'active':''}`} onClick={() => setTab('lich_su')}>
          📋 Lịch Sử Báo Lỗi {lichSu.filter(b=>b.trang_thai==='cho_xu_ly').length > 0 && <span className="xe-badge-count">{lichSu.filter(b=>b.trang_thai==='cho_xu_ly').length}</span>}
        </button>
      </div>
          ...
          <div className="card-body" style={{padding:0}}>
            {lichSu.length === 0 ? (
              ...
              <table className="data-table">
                <thead><tr><th>#</th><th>Xe</th><th>Tiêu đề</th><th>Mức độ</th><th>Trạng thái</th><th>Ghi chú xử lý</th><th>Ngày báo</th></tr></thead>
                <tbody>
                  {lichSu.map((bl, i) => (
                    <tr key={bl.id}>
                      <td>{i+1}</td>
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  {form.muc_do === 'nghiem_trong' && (
                    <p style={{fontSize:12,color:'#e53e3e',marginTop:6}}>⚠️ Xe sẽ tự động chuyển sang trạng thái "Hỏng" và không thể phân công cho buổi học mới.</p>
                  )}
                </div>
                <div className="form-group">
                  <label>Tiêu đề *</label>
                  <input value={form.tieu_de} onChange={e=>setForm({...form,tieu_de:e.target.value})} placeholder="VD: Lốp xe bị xẹp, Đèn pha không sáng..." required />
                </div>
                <div className="form-group">
                  <label>Mô tả chi tiết *</label>
                  <textarea rows={4} value={form.mo_ta} onChange={e=>setForm({...form,mo_ta:e.target.value})} placeholder="Mô tả chi tiết tình trạng hư hỏng, thời điểm phát hiện..." required />
                </div>
              </div>
